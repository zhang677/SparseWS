#include "benchmark/DCSC_DCSR/DCSC_DCSR_count_iter.h"
#include <iostream>
#include <time.h>
#include <string>
#include <fstream>
...
void count_iter_outer_CC_noT(const string filename1, const string filename2, const string& result_name, pid_t pid) {
    taco_tensor_t C;
    vector<int> indptr;
    vector<int> indices;
    ...
    int iterations = DCSC_DCSR_iteration(&A, &B, &C);
    std::cout << result2[0] << "," << iterations << "," << pid << std::endl;
    std::ofstream outfile;
    outfile.open(result_name, std::ios_base::app);
    outfile << result2[0] << "," << iterations << std::endl;
    outfile.close();
}
...
int main(int argc, char* argv[]) {
    ...
    const string result_name = (argc > 3) ? argv[3] : "./data/test.csv";

    pid_t pid = getpid();


    count_iter_outer_CC_noT(filename1, filename2, result_name, pid);
