...
int32_t TryInsert_coord_t(bool* insertFail, int32_t accumulator_size, int32_t i, int32_t k, float val) {
  if (accumulator_size == acc_capacity) {
    *insertFail = true;
    return Sort_t(acc_capacity, false);
  } else {
    w_accumulator_index[accumulator_size] = accumulator_size;
    w_accumulator.crd[0][accumulator_size] = i;
    w_accumulator.crd[1][accumulator_size] = k;
  ...
  for (int32_t j = 0; j < B1_dimension; j++) {
    for (int32_t iA = A2_pos[j]; iA < A2_pos[j+1]; iA++) {
      ...
      for (int32_t kB = B2_pos[j]; kB < B2_pos[j+1]; kB++) {
        ...
        #pragma omp critical 
        {
          w_accumulator_size = TryInsert_coord_t(&w_insertFail, w_accumulator_size, i, k, A_vals[iA] * B_vals[kB]);
          if (w_insertFail) {
...
void CSC_CSR_coord(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t* C, int32_t w_cap, bool print = false) {
