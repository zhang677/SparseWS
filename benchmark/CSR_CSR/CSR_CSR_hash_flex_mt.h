    ...
    for (int32_t jA = A2_pos[i]; jA < A2_pos[i+1]; jA++) {
      ...
      for (int32_t kB = B2_pos[j]; kB < B2_pos[(j + 1)]; kB++) {
        ...
        if (acc_array_id == 0) {
          ...
          TryInsert_hash(w_insertFail, &w_accumulator_0, w_point, (A_vals[jA] * B_vals[kB]), C2_dimension);
        } else {
          TryInsert_hash(w_insertFail, &w_accumulator_1, w_point, (A_vals[jA] * B_vals[kB]), C2_dimension);
        }
