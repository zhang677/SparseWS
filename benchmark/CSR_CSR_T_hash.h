  ...
  for (int32_t i = 0; i < A1_dimension; i++) {
    ...
    for (int32_t jA = A2_pos[i]; jA < A2_pos[(i + 1)]; jA++) {
      ...
      for (int32_t kB = B2_pos[j]; kB < B2_pos[(j + 1)]; kB++) {
        int32_t k = B2_crd[kB];
        w_point[0] = k;
        w_accumulator_size = TryInsert_hash(w_insertFail, w_accumulator, w_accumulator_size, w_accumulator_capacity, w_point, (A_vals[jA] * B_vals[kB]));
        if (w_insertFail[0]) {
...
void CSR_CSR_T_hash(const string& A_name, const string& B_name, taco_tensor_t* C, bool print = false) {
