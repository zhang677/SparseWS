...
int compute(taco_tensor_t *C, taco_tensor_t *A, taco_tensor_t *B, int32_t w_accumulator_capacity) {
      ...
      for (int32_t kB = B2_pos[j]; kB < B2_pos[(j + 1)]; kB++) {
        ...
        if (w_insertFail[0]) {
          ...
          if ((all_array_id == 0 && w_accumulator.numel + w_all_size > w_all_capacity_1) ||
            ...
            if (all_array_id == 1) {
              std::cout << "Realloc " << w_all_capacity_0 << "->" << w_accumulator.numel + w_all_size << ": ";
              ...
              std::cout << "Realloc " << w_all_capacity_1 << "->" << w_accumulator.numel + w_all_size << ": ";
          ...
          TryInsert_hash(w_insertFail, &w_accumulator, w_point, (A_vals[iA] * B_vals[kB]), B2_dimension);
          std::cout << "Refresh " << refresh_times << " : " << timer.elapsed() << "s" << std::endl;
          timer.reset();
        }
      }
    }
  ...
  if (w_accumulator.numel > 0) {
    ...
    all_array_id ^= 1;
    std::cout << "Output nnz: " << w_all_size << std::endl;
  }
  std::cout << "Tail: " << timer.elapsed() << "s" << std::endl;
  timer.reset();
  if (all_array_id == 0) {
    w1_crd = w1_crd_0;
    w2_crd = w2_crd_0;
  ...
  C->vals = (float*)w_vals;
  std::cout << "Compress: " << timer.elapsed() << "s" << std::endl;
  return 0;
}
