...
if __name__ == '__main__':
  ...
  if args.tiles:
      ...
      exist_mtxnames = [fname.replace("-t.mtx", ".mtx") for fname in os.listdir(out_dirname)]
      ...
      out_mtx_files = [os.path.join(out_dirname, fname.replace(".mtx", "-t.mtx")) for fname in mtxnames]
      ...
      for i, ten in tqdm(enumerate(tensors)):
           ...
           print(out_name)
           trans = inputCache.load(ten, False).transpose()
           outputStore.store(trans, out_name)
  else:
      tensor = SuiteSparseTensor(os.path.join(in_dirname, args.name + ".mtx"))
      trans = inputCache.load(tensor, False).transpose()
      outputStore.store(trans, out_name)
