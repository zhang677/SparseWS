...
int compute(taco_tensor_t *C, taco_tensor_t *A, taco_tensor_t *B) {
  ...
  for (int32_t pC = 0; pC < C2_pos[C1_dimension]; pC++) {
    C_vals[pC] = 0.0;
  }
...
int assemble(taco_tensor_t *C, taco_tensor_t *A, taco_tensor_t *B) {
  ...
  for (int32_t i = 0; i < A1_dimension; i++) {
    ...
    for (int32_t k = 0; k < B2_dimension; k++) {
      ...
      while (jA < pA2_end0 && jB < pB2_end0) {
        ...
        if (jA0 == j && jB0 == j) {
          int32_t pC2 = C2_pos[i];
          C2_pos[i] = C2_pos[i] + 1; // Assumes that non-zeros are added to different columns
          C2_crd[pC2] = k;
        }
        jA += (int32_t)(jA0 == j);
        jB += (int32_t)(jB0 == j);
  ...
  free(C2_nnz);
  print_array<int32_t>(C2_pos, (C1_dimension+1));
  print_array<int32_t>(C2_crd, nnz);
