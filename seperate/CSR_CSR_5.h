...
int32_t TryInsert(bool* insertFail, wspace* accumulator, int32_t accumulator_size, int32_t accumulator_capacity, int32_t* crds, float val, int hashkey) {
  int step = 0;
  wspace tmp;
  tmp.crd[0] = crds[0];
  tmp.crd[1] = crds[1];
  ...
  for (int32_t i = 0; i < A1_dimension; i++) {
    for (int32_t jA = A2_pos[i]; jA < A2_pos[i+1]; jA++) {
      ...
      for (int32_t kB = B2_pos[j]; kB < B2_pos[j+1]; kB++) {
        ...
        int w_hashkey = hash_func(i * B2_dimension + k, w_accumulator_capacity);
        w_accumulator_size = TryInsert(&insertFail, w_accumulator, w_accumulator_size, w_accumulator_capacity, C_crds, A_vals[jA] * B_vals[kB], w_hashkey);
        // No space to insert. Do contraction
        if (insertFail) {
          // Sort
          qsort(w_accumulator, w_accumulator_size, sizeof(wspace), esc_cmp);
