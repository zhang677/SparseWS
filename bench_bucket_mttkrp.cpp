#include "benchmark/MTTKRP/COO_CSF_DCSR_DCSR_coord_bucket.h"
#include <iostream>
#include <time.h>
#include <string>
#include <fstream>
...
void bench_mttkrp_dcsf_dcsr_dcsr_hash(const string filename1, const string filename2, const string filename3, const int repeat, const int warmup, vector<int>& dims, const string& result_name) {
    ...
    // int w_cap = pow(2,int(log2(dims[2] * ncol * 0.4))); // Good for sparsity = 0.1
    int w_cap = pow(2,int(log2(dims[2] * ncol * 0.2))); // heuristic
    printf("w_cap: %d\n", w_cap);
