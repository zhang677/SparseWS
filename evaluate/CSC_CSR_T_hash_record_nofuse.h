...
int transpose(taco_tensor_t *A, taco_tensor_t *B) {
  ...
  A->vals = (float*)A_vals;
  A->vals_size = B->vals_size;
  return 0;
}

double CSC_CSR_T_hash_nonfuse(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t* C_noT, taco_tensor_t* C, int w_cap, int& collisions) {
